...
console.log("Filtertype est : ", Filtertype)
    ...
    window.history.replaceState(null, "", `${window.location.pathname}?${params.toString()}`);
      ...
      const {data, isPending} = useGetClients(currentPage, "00001", Filtertype?Filtertype:type)

      const exportToExcel = async () => {
        let allData: string | any[] = [];
        let page = currentPage;
        let totalPages = 1; // Initialiser avec 1
      
        // On récupère d'abord le total de pages à partir des données
        const fetchData = async () => {
          const response = await useGetClients(currentPage, "00001", Filtertype ? Filtertype : type);
          return response?.data;
        };
      
        // Récupérer les données pour chaque page
        while (page <= totalPages) {
          console.log("page est : ", page)
          console.log("total est : ", totalPages)
          const data = await fetchData();
          allData = [...allData, ...data!.results];
          console.log("allData : ", allData)
          totalPages = Math.ceil(data!.count / pageSize); // Calculer le total des pages
          page++;
        }
      ...
      const exportToPDF = async () => {
        let allData: any[] = [];
        let page = currentPage;
        let totalPages = pageSize; // Initialiser avec 1
        ...
        doc.text("Liste des Clients", pageSize, page);
